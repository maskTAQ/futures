  ...
  if (subTitle) {
    return (
      <View style={{ flex: 1, }}>
        ...
        <View style={styles.titleWrapper}>
          <Text style={[styles.titleText, style, { fontSize: 12 }]}>{subTitle}</Text>
        </View>
      </View>
    )
  } else {
    ...
    return (
      <View style={[styles.container, style]}>
        ...
        <View style={styles.navigationContainer}>
          <Button style={[styles.item, styles.leftItem]} onPress={onLeftPress}>
            {LeftComponent !== undefined || <Icon size={20} source={leftIconSource} />}
          </Button>
          <View style={styles.title}>{titleComponent || renderTitle(styles, title, subTitle, titleStyle)}</View>
